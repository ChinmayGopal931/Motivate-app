...
function PromiseForm(props) {
  ...
  const Contract = new web3.eth.Contract(
    abi,
    "0x8E96E9B6bCB3DA7e7459f3115B4D4Ca364050429"
  );
  ...
  const createPromise = async (event) => {
    ...
    const endTimeStamp = Math.floor(
      nowTimestamp.setDate(nowTimestamp.getDate() + parseInt(promiseDeadline)) /
        1000
    );
    const tx = Contract.methods
      .createPromise(
        promiseTask,
        web3.utils.toWei(promiseAmount, "ether"),
        friend,
        endTimeStamp
      )
      .send({
        ...
        >
          <FormControl>
            <HStack w={600} justify="center" align="center">
              ...
              <InputGroup maxW={40}>
                <Input
                  onChange={({ target }) => setPromiseAmount(target.value)}
                  value={promiseAmount}
                  type="number"
                  isRequired
                />
              ...
              <FormLabel fontSize={22}>that</FormLabel>
            </HStack>
          </FormControl>
          <FormControl>
            <Textarea
              isRequired
              onChange={({ target }) => setPromiseTask(target.value)}
              value={promiseTask}
              size="lg"
              resize="vertical"
              rows="3"
          ...
          <FormControl>
            <HStack justify="center" align="center">
              ...
              <InputGroup maxW={40}>
                <Input
                  onChange={({ target }) => setPromiseDeadline(target.value)}
                  value={promiseDeadline}
                  type="number"
                  isRequired
                />
              </InputGroup>
              <FormLabel fontSize={22}>days.</FormLabel>
            </HStack>
          </FormControl>
          <FormControl>
            <Input
