...
function Promises(props) {
  ...
  const [isLoadingPromisesToFulfill, setIsLoadingPromisesToFulfill] =
    useState(true);
  const [isLoadingPromisesToVerify, setIsLoadingPromisesToVerify] =
    useState(true);
  ...
  const getPromisesToFulfill = useCallback(async () => {
    ...
    const result = await Contract.methods
      .getPendingPromises()
      .call({ from: props.currentAccount });
    const promises = [];
          ...
          <Table>
            ...
            <Tbody>
              {promisesToFulfill.map((promise) => {
                return (
                  <Tr key={promise["0"]}>
                    <Th>{promise["promiseTask"]}</Th>
                    <Th>
                      <Tag>
                        {`${new Date(
                          parseInt(promise["endTime"]) * 1000
